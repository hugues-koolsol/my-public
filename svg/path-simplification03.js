...
const findRoot = (curve, point, u) => {
    ...
    const pt1 = evaluate(2, curve1, u), pt2 = evaluate(1, curve2, u), diff = pt._subtract(point), df = pt1._dot(pt1) + diff._dot(pt2);
    // u = u - f(u) / f'(u)
...
const findMaxError = (points, first, last, curve, u) => {
    ...
    for (let i = first + 1; i < last; i++) {
        const P = evaluate(3, curve, u[i - first]); // hugues 3
...
//        console.log( 'skipLine=' , skipLine )
        ...
        if (first) {
            if(fact==1){
             bez2.push(['p' , null , null , parseInt(curX,10), parseInt(curY,10) , null , null , 'aigu' ]);
            }else{
             bez2.push(['p' , null , null , parseInt(curX*fact,10)/fact, parseInt(curY*fact,10)/fact , null , null , 'aigu' ]);
            }
//            bez.push([curX, curY , 's']);
//            bez3.push([curX, curY , 's']);
...
//                console.log('ici');
                if (!skipLine) {
                    const dx = curX - prevX;
                    const dy = curY - prevY;
//                    bez.push([dx, dy , dx === 0 ? 'v' : ( dy === 0 ? 'h' : 'l') ])
                ...
                if(fact==1){
                 bez2[bez2.length-1][5]=parseInt(outX,10);
                 bez2[bez2.length-1][6]=parseInt(outY,10);
                 bez2.push(['p' , parseInt(inX,10) , parseInt(inY,10) , parseInt(curX,10) , parseInt(curY,10) , null , null , 'aigu' ]);
                }else{
                 bez2[bez2.length-1][5]=parseInt(outX*fact,10)/fact;
                 bez2[bez2.length-1][6]=parseInt(outY*fact,10)/fact;
                 bez2.push(['p' , parseInt(inX*fact,10)/fact , parseInt(inY*fact,10)/fact , parseInt(curX*fact,10)/fact , parseInt(curY*fact,10)/fact , null , null , 'aigu' ]);
                }
                
//                bez3.push([parseInt(outX*1000)/1000  , parseInt(outY*1000)/1000  , '' ]);
//                bez3.push([parseInt(inX*1000)/1000   , parseInt(inY*1000)/1000   , '' ]);
//                bez3.push([parseInt(curX*1000)/1000  , parseInt(curY*1000)/1000  , '' ]);

//                bez.push([outX - prevX , outY - prevY , '' ]);
//                bez.push([inX - prevX  , inY - prevY  , '' ]);
//                bez.push([curX - prevX , curY - prevY , '' ]);
    ...
    return {path:parts.join('') , parts:parts , bez2:bez2};
};
